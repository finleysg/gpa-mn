...
export async function uploadPhotoAction(formData: FormData) {
    ...
    const variant =
        (formData.get("variant") as "desktop" | "mobile" | "default" | "inline") ?? "default"
...
export async function getInlinePhotosAction(photoType: "event" | "content", parentId: number) {
    const all = await getPhotos(photoType, parentId)
    return all.filter((p) => p.variant === "inline")
}
