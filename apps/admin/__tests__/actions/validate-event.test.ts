...
describe('validateEventData', () => {
  ...
  it('returns error when monthly recurrence has no endDate', () => {
    const errors = validateEventData({ ...validEvent, recurrence: 'monthly', endDate: null });
    expect(errors).toContain('End date is required for recurring events');
  });

  it('passes when recurring event has an endDate', () => {
    const errors = validateEventData({ ...validEvent, recurrence: 'weekly', endDate: '2026-12-31' });
