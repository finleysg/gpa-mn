...
Deploy an app (admin or web) to production by creating and pushing a git tag. The tag push triggers the GitHub Actions deployment workflow automatically.

## Tag Format

`{app}-v{yyyy}.{mm}.{dd}.{nn}`

- `{app}` is either `admin` or `web`
...
If the user specified the app (e.g., `/deploy admin` or `/deploy web`), use that.
Otherwise, ask the user which app to deploy using AskUserQuestion with options "admin" and "web".
...
- Link to monitor: https://github.com/finleysg/gpa-mn/actions
