...
export async function getPageBySectionAndSlug(section: string, slug: string) {
    const results = await withLatestVersion().where(
        and(
            eq(contentItems.contentType, "page"),
            eq(contentItems.slug, slug),
            eq(contentItems.archived, false),
        ),
    )

    const match = results.find(
        (r) => (r.version.data as Record<string, unknown>).section === section,
    )
...
export async function getContentItem(id: number) {
    const [result] = await withLatestVersion(eq(contentVersions.contentItemId, id)).where(
...
export async function revertToVersion(
    ...
    const [targetVersion] = await db
        .select()
        .from(contentVersions)
        .where(eq(contentVersions.id, targetVersionId))
